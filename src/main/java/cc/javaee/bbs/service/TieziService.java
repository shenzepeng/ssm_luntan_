...
public class TieziService {
	...
	public PageBean<Tiezi> findpagebybankuanid(Tiezi tiezi, PageBean<Tiezi> page) {
	...
	public PageBean<Tiezi> findpagebyCreateuserid(Tiezi tiezi, PageBean<Tiezi> page) {
		page.setBean(tiezi);
		int count = tieziDao.findpagecount(page);
		page.setTotalRecords(count);
		List<Tiezi> list = tieziDao.findpage(page);
	...
	public Map<String, Object> findbytieziid(String id) {
		...
		if (contenttxt != null && contenttxt.length() > 200) {
			// 对页面描述(<meta name="description" content="${map.tiezi.contenttxt}" />
			// )的修改，截取并替换<>
